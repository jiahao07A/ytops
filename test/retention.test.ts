  ...
  const writeInventory = async (videoIds: string[]): Promise<void> => {
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        progress: {
          pages: 1,
          items: videoIds.length,
          videoItems: videoIds.length,
        },
...
describe("留存曲线同步任务", () => {
  it("首次留存同步对库存全部视频各建立一条全历史留存曲线", async () => {
    await withRetentionFixture(
      async ({ configPath, store, writeInventory }) => {
  ...
  it("后续同步只请求新发现的视频", async () => {
    await withRetentionFixture(
      async ({ configPath, store, writeInventory }) => {
        ...
        expect(
          secondProvider.requests.map((request) => request.videoId),
        ).toEqual(["video-003"]);
        expect(second.state.completedVideoIds).toEqual([
          "video-001",
          "video-002",
...
describe("留存曲线官方适配器", () => {
  ...
  it("整个响应没有数据行时视为该视频暂无留存曲线", async () => {
    const provider = new GoogleRetentionProvider(
      async () =>
        new Response(
          JSON.stringify({
            columnHeaders: [
