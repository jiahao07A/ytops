...
Revenue metrics (`estimatedRevenue`) stay gated behind an explicit opt-in (ADR 0003): enable it with `ytops config set-global --analytics-revenue-opt-in true` (or `--channel-analytics-revenue-opt-in` for one channel), then re-authenticate with `auth-start --analytics --analytics-revenue` so the grant includes `yt-analytics-monetary.readonly`. Opted-in core syncs request revenue with an explicit `currency=USD`; audience breakdown groups never carry revenue metrics, and on-demand breakdowns reject revenue queries locally when the opt-in is absent. Without the opt-in the scope is never requested, and the coverage matrix reports `analytics.revenue` as qualification-limited instead of faking zeros. Official documentation is contradictory about channel-level revenue availability: run a real-channel probe (opt-in, re-auth, one `analytics-sync`, inspect `evidence/`) and record the outcome before promising channel-level revenue behavior. Revenue figures are estimates that the official source may adjust at month end, so already-synced revenue rows can shift after the fact; keep the data-as-of semantics when reporting them. `ops doctor --config <path>` reports only the opt-in status, never config values.
...
Core sync also stores `dislikes`, `subscribersGained` and `subscribersLost`; note that at video level the subscriber metrics count only activity on that video's watch page, while `likes`/`dislikes` belong to the video wherever the engagement happened. Revenue figures are estimates that the official source may adjust at month end. Like/dislike ratio and RPM (per thousand engaged views) exist only as read-time derivations via `analytics-read --derived`.

Real-channel probe verdicts (2026-08-30, monetized channel): channel-level `estimatedRevenue` IS available via the query API (contradicting the docs note), synced rows carry explicit `currency=USD`. Audience groups follow hard official constraints — traffic-source and subscribed-status groups support the viewer metrics per day; the geography group (country) and the demographics group (ageGroup+gender × `viewerPercentage`) do NOT support the `day` dimension and return window aggregates instead; `engagedViews`/`averageViewDuration` are rejected on geography. Reporting jobs.create for reach reports requires a `name`; a duplicate job returns 409 and the CLI reuses the existing job, and the first report can take up to 48 hours to appear. Retention sync marks a video completed even when the official response contains no points (empty curve), and the official API intermittently returns 5xx bursts on heavier queries — every sync is resumable, so re-running the same command later is the intended recovery.
