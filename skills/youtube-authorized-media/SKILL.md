...
description: 仅为用户明确拥有或获授权下载和处理的 YouTube 内容准备视频、音频或字幕文件。用户请求授权下载、授权字幕获取或可复现的本地媒体接入流程时使用。不要用于浏览、私有频道操作、Cookie 提取或未确认授权的第三方内容。
---
...
- Do not use `--cookies-from-browser`, cookie files, OAuth refresh tokens, or user-level `yt-dlp` configuration.
